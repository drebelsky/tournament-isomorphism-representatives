...
void iterate_possibilites(std::ostream &out, int n, long start, long stop) {
  ...
  for (long graph = start; graph < stop; graph++) {
    ...
    populate_degrees_3(in_degrees, out_degrees, in_degrees_2, out_degrees_2,
                       in_degrees_3, out_degrees_3, edges, n, graph);
    ...
    for (size_t rep_ind = 0; rep_ind < representatives.size(); rep_ind++) {
      ...
      int isometric_permutation =
          is_isometric(edges, n, representatives[rep_ind], graph);
      if (isometric_permutation) {
        out << graph << ": " << representatives[rep_ind] << " ("
...
int main(int argc, char *argv[]) {
  ...
  if (argc != 4) {
    cout << "Usage: " << argv[0]
         << " <n> <start (inclusive)> <stop (exclusive)>" << std::endl;
    return 1;
  }

  int n = std::stoi(argv[1]);
  long start = std::stol(argv[2]);
  long stop = std::stol(argv[3]);

  std::ostringstream filename;
  filename << n << "_" << start << "-" << stop << ".txt.new";
  std::ofstream output(filename.str());

  iterate_possibilites(output, n, start, stop);
  return 0;
}
